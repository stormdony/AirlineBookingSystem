 ...
 *
 */
public class CustomerMainQueryUi implements ActionListener {
	...
	public CustomerMainQueryUi(String Cid, String begin, String start, String destination, String end)
			throws Exception {
		id = Cid;
		ResultSet rs = MainQueryUtils.CustomerQueryAll(begin, start, destination, end);
		int i = 0;
		try {
			while (rs.next()) {
				i++;
		...
		if (i > 0) {
			...
			flightList = MainQueryUtils.cusQueryFlight(begin, start, destination, end);
			int k = 0;
			// while (Set.next()) {
			for (int j = 0; j < flightList.size(); j++) {
				...
				System.out.println(id + "---" + Fname + "--" + Fstart + "--" + Fdestination + "---" + Fbegin + "--"
						+ Fend + "--" + grade + "--" + price + "--" + discount);
				// 创建信息框
